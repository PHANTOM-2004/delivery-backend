...
delivery_merchant_application(
  ...
  `updated_at` datetime not null,
  `deleted_at` datetime,
  `status` tinyint default 0 not null comment '0 代表不通过审核，1 代表未审核，2代表通过审核',
...
delivery_merchant(
  ...
  foreign key (application_id) references delivery_merchant_application(id) on delete set null,
  primary key(id)
